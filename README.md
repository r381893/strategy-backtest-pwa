# Strategy Backtest PWA

高級回測系統 Pro 的 PWA 版本，採用前端 + Render 後端 + Firebase 架構。

## 專案結構

```
pwa/
├── frontend/               # 前端 PWA (GitHub Pages)
│   ├── index.html         # 主頁面
│   ├── css/style.css      # 樣式檔
│   ├── js/
│   │   ├── app.js              # 主應用邏輯
│   │   ├── firebase-config.js  # Firebase 設定
│   │   ├── file-parser.js      # Excel 解析
│   │   └── chart.js            # 圖表繪製
│   ├── manifest.json      # PWA manifest
│   └── sw.js              # Service Worker
├── backend/               # Render 後端
│   ├── app.py             # Flask API
│   ├── backtest.py        # 回測引擎
│   ├── optimizer.py       # 參數優化器
│   └── requirements.txt   # Python 依賴
└── .github/workflows/     # CI/CD
    └── deploy.yml         # GitHub Pages 自動部署
```

## 功能

- 📁 **資料預覽**：上傳 Excel 檔案，自動識別日期/價格欄位
- ⚙️ **策略設定**：設定回測參數（槓桿、手續費、策略類型等）
- 📊 **回測報表**：顯示績效指標、淨值曲線、回撤分析
- 📋 **交易明細**：交易統計摘要和歷史交易紀錄
- 🔍 **參數優化**：自動搜尋最佳策略參數組合
- 💾 **已儲存策略**：查看 Firebase 中儲存的最佳策略

## 部署

### 前端 (GitHub Pages)

1. 將 `frontend` 資料夾推送到 GitHub
2. 啟用 GitHub Pages（Settings > Pages > Source: GitHub Actions）
3. 每次推送到 main 分支會自動部署

### 後端 (Render)

1. 在 Render 建立新的 Web Service
2. 連接 GitHub repository
3. 設定：
   - Root Directory: `backend`
   - Build Command: `pip install -r requirements.txt`
   - Start Command: `gunicorn app:app`
4. 部署後更新前端 `app.js` 中的 `API_BASE_URL`

### Firebase

使用現有的 `backtesting-system-pro` Firebase 專案：
- Database URL: `https://backtesting-system-pro-default-rtdb.asia-southeast1.firebasedatabase.app/`

> ⚠️ 記得在 `frontend/js/firebase-config.js` 中填入正確的 Firebase API Key

#### 資料庫規則設定

為了讓 PWA 前端可以讀寫資料，需要在 Firebase Console 設定 Realtime Database 規則：

1. 前往 [Firebase Console](https://console.firebase.google.com/)
2. 選擇 **Backtesting System Pro** 專案
3. 左側選單點擊「**Realtime Database**」
4. 點擊「**規則**」標籤
5. 將規則改為：

```json
{
  "rules": {
    "best_strategies": {
      ".read": true,
      ".write": true
    }
  }
}
```

6. 點擊「**發布**」

> ⚠️ **安全提醒**：此設定允許任何人讀寫 `best_strategies` 節點。若需要更嚴格的安全控制，可加入 Firebase Authentication。

## 本地開發

### 前端
```bash
cd frontend
# 使用任何 HTTP 伺服器
python -m http.server 8000
```

### 後端
```bash
cd backend
pip install -r requirements.txt
python app.py
```

## 技術棧

- **前端**：Vanilla JS + Plotly.js + SheetJS + Firebase SDK
- **後端**：Flask + Pandas + NumPy
- **部署**：GitHub Pages + Render
- **資料庫**：Firebase Realtime Database

---

## 設計邏輯

### 架構選擇

```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│  GitHub Pages   │────▶│     Render      │────▶│    Firebase     │
│   (前端 PWA)    │     │  (Python API)   │     │  (Realtime DB)  │
└─────────────────┘     └─────────────────┘     └─────────────────┘
```

**為什麼選擇這個架構？**

| 元件 | 選擇理由 |
|------|----------|
| **GitHub Pages** | 免費、自動 HTTPS、與 Git 整合、支援 PWA |
| **Render** | 免費 tier、支援 Python、自動部署、CORS 友好 |
| **Firebase** | 即時同步、免費額度大、前後端都能存取 |

### 前後端分離的優勢

1. **獨立擴展**：前端和後端可以分別擴展和更新
2. **快速迭代**：前端 UI 更新不需重新部署後端
3. **離線支援**：PWA 可以在離線時顯示快取的策略資料
4. **成本控制**：計算密集的回測在後端執行，前端保持輕量

### 回測邏輯（後端計算）

```
上傳 Excel → 解析資料 → API 送至後端 → Python 回測計算 → 返回結果
```

**選擇後端計算的原因**：
- 回測涉及大量數值計算（均線、損益、MDD 等）
- Python 的 Pandas/NumPy 效能優於 JavaScript
- 原有程式碼可直接移植，減少轉換錯誤
- 支援更大的資料集（數萬筆歷史資料）

### 策略類型

| 策略 | 進場邏輯 | 出場邏輯 |
|------|----------|----------|
| **永遠做多** | 第一天全倉做多 | 不出場（持有到底） |
| **單均線** | 價格上穿 MA | 價格下穿 MA |
| **雙均線** | 快線上穿慢線 | 快線下穿慢線 |

### 績效指標計算

- **CAGR**：`(最終淨值 / 初始資金)^(1/年數) - 1`
- **MDD**：歷史最大回撤百分比
- **Sharpe**：`(年化報酬 - 無風險利率) / 年化標準差`
- **Calmar**：`CAGR / MDD`

### Firebase 資料結構

```json
{
  "best_strategies": {
    "資產名稱_回測區間": {
      "asset": "加權指數",
      "strategy": "永遠做多",
      "leverage": 2.0,
      "total_return": 0.85,
      "cagr": 0.12,
      "mdd": 0.25,
      "saved_at": "2024-12-27 13:30:00"
    }
  }
}
```

---

## 原版 Streamlit 對照

本 PWA 版本完整移植了原 Streamlit 應用的所有功能：

| Streamlit 原版 | PWA 版本 |
|----------------|----------|
| `st.file_uploader` | SheetJS + Drag & Drop |
| `st.number_input`, `st.selectbox` | HTML form elements |
| `st.plotly_chart` | Plotly.js |
| `st.dataframe` | HTML Table |
| `st.tabs` | Tab Navigation |
| Firebase Admin SDK | Firebase Web SDK |
